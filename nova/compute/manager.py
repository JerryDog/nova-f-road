...
class ComputeManager(manager.Manager):
    ...
    def _prep_resize(self, context, image, instance, instance_type,
        ...
        vm_state = instance.vm_state
        LOG.info('Stashing vm_state: %s', vm_state, instance=instance)
    ...
    def prep_resize(self, context, image, instance, instance_type,
        ...
        """
        LOG.info('resize_step_1_manager_prep_resize')
        if node is None:
            node = self.driver.get_available_nodes(refresh=True)[0]
            LOG.debug("No node specified, defaulting to %s", node,
                      instance=instance)
    ...
    def resize_instance(self, context, instance, image,
        ...
        LOG.info('resize_step_2_manage_resize_instance')
        quotas = objects.Quotas.from_reservations(context,
                                                  reservations,
                                                  instance=instance)
        with self._error_out_instance_on_exception(context, instance,
            ...
            LOG.info('instance.host: %s' % instance.host)
            LOG.info('instance.node: %s' % instance.node)
    ...
    def _finish_resize(self, context, instance, migration, disk_info,
        ...
        LOG.info('old_instance_type_id: %s' % old_instance_type_id)
        LOG.info('new_instance_type_id: %s' % new_instance_type_id)
        ...
        try:
            LOG.info('BBBBBBBB finish_migration')
